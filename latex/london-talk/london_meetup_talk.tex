...
%	\item Closely related to constructing priors
	\item Experiments are hard.  Sampling from priors on deep nets helps understand them without reference to a specific dataset, loss function, or training method.
	\item We'll visualize and analyze samples:
\begin{tabular}{cccc}
...
\begin{itemize}
	...
	\item Avoids problem of unit saturation (activation functions usually sigmoidal).
	\item Maybe we can learn something just from looking at draws? Each draw from neural net prior defines a (vector-valued) function $\vy = \vf(\vx)$.
\end{itemize}
}
...
\begin{itemize}
	...
	\only<4>{\gpdrawbox{4}}
	\only<5>{\gpdrawbox{5}}
	\only<6>{\gpdrawbox{6}\\\vspace{-0.1cm}Density concentrates along filaments.}
\end{itemize}
}
...
\begin{itemize}
	...
	\only<6>{\gpdrawboxcon{6}\\\vspace{-0.5cm}Density becomes more complex but remains 2D.}%
\end{itemize}
}
...
2012; Srivastava, 2013)}.
	...
	\item What do we get from dropout in one-layer GPs?
\end{itemize}
}
...
\frame[plain]{
\frametitle{Dropout in a shallow Gaussian process}
...
\frame[plain]{
\frametitle{Covariance after dropout}
...
%\end{tabular}
\begin{tabular}{ccc}
Original & & After dropout\\
$\displaystyle \prod_{d=1}^D k_d(\vx_d, \vx_d')$ & & $\displaystyle \sum_{\vr \in \{0,1\}^D}  \prod_{d=1}^D k_d(\vx_d, \vx_d')^{r_d}$ \\
\includegraphics[width=0.3\textwidth]{figures/3d-kernel/3d_add_kernel_3} & \phantom{m} &
...
%	\vspace{\baselineskip}
	\item Building priors over functions can shed light on architectures choices or regularization strategies in a data-independent way.
	\item Dropout makes output similar even if inputs change a little.
