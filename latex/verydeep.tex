...
Choosing appropriate architectures and initialization strategies is crucial to good performance of deep networks.  To shed light on this problem, we analyze the analogous problem of constructing useful priors on deep networks.
...
There exists a simple correspondence between one-layer MLPs and \gp{}s \cite{neal1995bayesian}.  \gp{} priors can be viewed as a prior on neural networks with infinitely many hidden units.  More precisely, for any model of the form
%
\begin{align}
...
This second view lets us compare \gp{} models to multilayer perceptrons more directly, examining the activations and shapes of the finite number of basis functions at each layer.
...
In this section, we characterize the limiting distribution of an arbitrarily deep, one-dimensional gp{}.  Doing so will help characterize the ways in which a deep \gp{} is non-Gaussian, and the ways in which lower layers affect the computation performed by higher layers.  Figure \ref{fig:deep_draw_1d} shows a draw from a 1D deep \gp{} prior, at varying depths.
%
...
\centering
\begin{tabular}{cccc}
\hspace{-0.1in} One layer & \hspace{-0.2in} 2-Layer Composition & \hspace{-0.2in}  5-Layer Composition & \hspace{-0.25in} 10-Layer Composition \\
\hspace{0.03in}
\onedsamplepic{1} &
\onedsamplepic{2} &
\onedsamplepic{5} &
...
\log \left| \frac{\partial f^{1:L}(x)}{\partial x} \right| 
 = \sum_{i=1}^L \log \left| \frac{\partial f^i(x)}{\partial x} \right| \implies
...
%
\begin{proof}
The Jacobian of the vector-valued function $f(\vx)$ is a matrix $J$ whose $(i,j)^{\textnormal{th}}$ element is
...
Because we've assumed that the \gp{} on each output dimension $f_d(\vx) \sim \GP$ are \iid{}, it follows that for a given $\vx$, each row of $J$ is \iid{}
...
\begin{tikzpicture}[pile/.style={thick, ->, >=stealth'}]
    \node[anchor=south west,inner sep=0] at (0,0) {
    	\includegraphics[clip, trim = 0cm 12cm 0cm 0.0cm, width=0.45\columnwidth]{figures/hidden_good}
    };
...
%\hspace{-0.2in}
\includegraphics[trim=2mm 0mm 2mm 6.4mm, clip, width=0.315\columnwidth]{figures/spectrum/layer-#1}} 
\begin{figure}[h]
\centering
\begin{tabular}{ccc}
...
\end{figure}
%
Figure \ref{fig:filamentation} demonstrates a related pathology that arises when composing functions to produce a deep density model.  The density in observed space eventually becomes locally concentrated onto one-dimensional manifolds, or \emph{filaments}.
...
Follow a suggestion from \cite{neal1995bayesian}, we can fix the pathologies exhibited in figures \ref{fig:filamentation} and \ref{fig:deep_map} by simply making each layer of depend not only on the output of the previous layer, but also on the original input $\vx$.  That is, $\forall L, f^{1:L}(\vx) = f^{L}( f^{1:L-1}(\vx), \vx)$. Draws from the resulting priors are shown in figures \ref{fig:deep_draw_1d_connected}, \ref{fig:deep_map_connected} and \ref{fig:no_filamentation}.
%
\begin{figure}[h!]
\centering
\begin{tabular}{cccc}
...
%& 6 layers \\ & largest , for unconnected layers & largest, for connected layers
\end{tabular}
\caption{One-dimensional draws from a deep \gp{} prior with each layer connected to the input. Even after many layers, the functions remain smooth in some regions, while varying rapidly in other regions.}
...
\section{Arbritarily Deep Kernels}
...
Kernels can capture many other types of generalization, such as translation and rotation invariance in images \cite{kondor2008group}.  \cite{SalHin08} even uses a deep neural network to learn feature transforms for kernels, which learn invariances in an unsupervised manner.  
%
...
Variational inference in deep Gaussian processes was developed by \cite{damianou2012deep}, who also analyzed the effect of ARD in these models.
%
