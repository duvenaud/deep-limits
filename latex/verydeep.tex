...
%\aistatstitle{Building priors on very deep neural networks}
...
\end{tabular}
\caption{Normalized singular value spectrum of the Jacobian of a deep GP.  As the net gets deeper, the largest singular value dominates.
...
\end{figure}
%
To visualize this pathology in another way, figure \ref{fig:deep_map} illustrates the value that at each point in the input space is mapped to after successive warpings.  After 10 warpings, we can see that locally, there is usually only one direction that one can move in $\vx$-space in order to change the value of the function.
...
\includegraphics[trim=4mm 1mm 4mm 2.5mm, clip, width=0.475\columnwidth]{figures/spectrum/con-layer-#1}} 
\begin{tabular}{ccc}
 & 25 layers &  50 layers \\
...
\centering
\begin{tabular}{c}
\hspace{-0.3cm}\includegraphics[width=\columnwidth, clip, trim = 0.17cm 0.05cm 0.9cm 0.3cm]{figures/deep_kernel_connected}
...
\hspace{-0.5cm}\includegraphics[width=\columnwidth, clip, trim = 0cm 0.05cm 0.9cm 0.3cm]{figures/deep_kernel_connected_draws} \\
\end{tabular}
\caption{Draws using the deep input-connected kernel.  
}
...
We present two ways to gain intuition about this model class.

First, if each $k_d(\vx_d, \vx_d')$ has the form ${k(x, x') = g \left( \frac{x - x'}{\lengthscale} \right)}$, as does the \humble{SE} kernel \eqref{eq:se_kernel}, then an input dimension can be dropped out by setting its lengthscale $\lengthscale$ to $\infty$.
Thus, dropping out the inputs of a \gp{} corresponds to modifying the prior on the lengthscales to be a spike-and-slab prior, with mass on $w = \infty$.



Second, the dropout mixture has the same covariance as
\begin{align}
...
In this work, we attempted to gain insight into the properties of neural networks by comparing the sorts of functions likeley to be obtained by under different choices of priors.
...
Finally, we derived the models obtained by performing dropout on deep Gaussian processes, finding a tractable approximation to exact dropout in one-layer \gp{}s.
