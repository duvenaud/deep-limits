...
the features $\hPhi(\vx)$ of the \gp{} correspond to the hidden units of the \MLP{}.
...
%Since the fixed hidden units
% in the(given by Mercer's theorem) 
%$\hPhi^{(\layerindex)}(\vx)$ depend only on a linear projection of their inputs $\hPhi(\vx) = \sigma \left( \vb^{(\layerindex)} + \vW^{(\layerindex)}f^{(\layerindex-1)}(\vx) \right)$, 
...
\mappic{0} & \mappic{1} \\
 2 Layers: $\vy = f^{(1:2)}(\vx)$ & 40 Layers: $\vy = f^{(1:40)}(\vx)$ \\%\\2 Layers: $\vy = f_1(f_2(\vx))$ \\
\mappic{2} & \mappic{40}
%\begin{tikzpicture}[pile/.style={thick, ->, >=stealth'}]
%    \node[anchor=south west,inner sep=0] at (0,0) {
%    	\mappic{40}
...
In this work, we attempted to gain insight into the properties of neural networks by examining the sorts of structures that different priors on functions give rise to.
