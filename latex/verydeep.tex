...
\vf_d^{(\ell)} & \simind \GPt{0}{k^\ell_d(\vx, \vx')}
\end{align}
%
%Although inference in these models is non-trivial,
These models can be derived as a special case of multi-layer perceptrons (\MLP{}s), and as such make canonical candidates for generative models of functions that closely relate to neural networks.
...
In an \MLP{}, the $n^{th}$ layer units are given by the recurrence:
%
\begin{align}
...
However, in a deep \gp{}, the $D$ outputs $\vf^{(n)}(\vx)$ in between each layer are weighted sums of the hidden units of the layer below, and the next layer's hidden units depend only on these $D$ outputs.
Thus deep \gp{}s have an extra set of layers that a \MLP{} doesn't have, shown in figure \ref{fig:architectures}c.

There are two ways to directly relate deep \gp{}s to \MLP{}s.
First, we can note that, if the hidden units in a deep \gp{} implied by Mercer's theorem $\phi^{(n)}(\vx)$ depend only on a linear projection of their inputs, as in the sigmoidal activation function $\phi(\vx) = \sigma \left( \vb^{(n)} + \vW^{(n)}f^{(n-1)}(\vx) \right)$, then we can simply substitute $\vf^{(n-1)}(\vx) = \vV^{(n-1)} \phi^{(n-1)}(\vx)$ to recover $\phi(\vx) = \sigma \left( \vb^{(n)} + \vW^{(n)} \vV^{(n-1)} \phi^{(n-1)}(\vx) \right)$.
Thus, we can ignore the intermediate outputs $\vf^{(n)}(\vx)$, and exactly recover an MLP with activation functions given by Mercer's theorem, but with rank-$D$ weight matrices 
%$W^{(n)} V^{(n-1)}$
 between layers.
...
Dropout is a method for regularizing neural networks \citep{srivastava2013improving}.
...
To maintain similar overall activation levels at test time, weights are multiplied by $\nicefrac{1}{p}$, or alternatively, features activations are multiplied by $\nicefrac{1}{p}$ during training.
At test time, the set of models defined by all possible ways of dropping-out neurons is averaged over in an approximate way.
...
In this section, we examine the priors on functions that result from performing dropout in the one-layer neural network implicitly defined by a \gp{}.


\subsection{Dropout on feature activations}

First, we examine the prior that results from randomly removing features $\phi(x)$ with probability $p$, in a one-hidden-layer neural network with an infinite number of features, given by \eqref{eq:one-layer-nn}.
If these features have a weight distribution with finite moments
$\expectargs{}{ \alpha_i} = 0, \varianceargs{}{\alpha_i} = \sigma^2$,
then the distribution of weights after each one has been dropped out with probability $p$ will be:
\begin{align}
\expectargs{}{ r_i \alpha_i} = 0, \varianceargs{}{r_i \alpha_i} = p^{-2} \sigma^2 \quad r_i \simiid \textnormal{Ber}(p)
\end{align}
However, after we increase the remaining activations to maintain the same expected activation, (by multiplying them by $p$), the resulting moments are again:
${\expectargs{}{ p r_i \alpha_i } = 0, \varianceargs{}{ p r_i \alpha_i} = \sigma^2}$.
%Since only the first two moments of the weight variance matter, the resulting \gp{} remain
Thus, performing dropout on feature activations of an infinitely-wide MLP does not change the model at all.
This could perhaps be expected, because the limit of MLPs corresponding to \gp{}s requires that no individual unit can have more than infintesimal contribution to the activations.
%Is there a better way to drop out features that would lead to robustness?

\subsection{Dropout on inputs}


If $k(\vx, \vx') = \prod_{d=1}^D k_d(\vx_d, \vx_d')$, then exact averaging over all possible ways of dropping out inputs results in a mixture of \gp{}s, each depending on only a subset of the inputs:
\begin{align}
...
This dropout mixture has the covariance as
\begin{align}
f(\vx) \sim \textnormal{\gp} \left(0, \frac{1}{2^D} \sum_{\vr \in \{0,1\}^D}  \prod_{d=1}^D k_d(\vx_d, \vx_d')^{r_d} \right).
\end{align}

This model exactly corrsponds to additive Gaussian processes \citep{duvenaud2011additive11}, a sum over $2^D$ functions, each depending on a different subset of the inputs.
Exact inference in these models can be performed in $\mathcal{O}(N^2 D^2) + \mathcal{O}(N^3)$.
...
%
\begin{align}
f(\vx) = \frac{1}{2}{K} \sum_{i=1}^K {\color{red}r_i} \alpha_i \phi_i(\vx) \quad {\color{red}r_i} \simiid \textnormal{Ber}(p)
\end{align}
