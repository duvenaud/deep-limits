...
%\hspace{0.2cm}
\vspace{1cm}\\
\normalsize Sheffield Workshop on Deep Probabilistic Models
...
\frame[plain]{
\frametitle{Designing neural nets}
\begin{itemize}
%	\item Deep neural networks define functions: $\vy = \vf(\vx)$
%	\item Closely related to constructing priors
	\item Neural nets require lots of design decisions whose implications hard to understand.
	%\item Why not just sample from priors on deep nets?
	\item We want to understand them without reference to a specific dataset, loss function, or training method.
	\item We can analyze different network architectures by looking at nets whose parameters are drawn randomly.
\end{itemize}
}
...
\frame[plain]{
\frametitle{Why look at priors if I'm going to learn everything anyways?}
\begin{itemize}
	\item When using Bayesian neural nets:
	\begin{itemize}
	\item Can't learn types of networks having vanishing probability under the prior.
\end{itemize}
	\item Even when non-probabilistic:
	\begin{itemize}
	\item Good prior $\rightarrow$ a good initialization strategy.
	\item Good prior $\rightarrow$ a good regularization strategy.
	\item Good prior $\rightarrow$ higher fraction of parameters specify reasonable models $\rightarrow$ easier optimization problem.
\end{itemize}
\end{itemize}
}
...
\frame[plain]{
\frametitle{Repeated Fixed Feature Mappings}
%\begin{tabular}{c|c}
...
%\hspace{-1.4cm}
    ...
    \foreach \name / \y in {1,2}%...,\numhidden}
        \path[yshift=0.5cm]
            node[hidden neuron, minimum size=\nodesize] (H4-\name) at (4*\layersep,-\nodesep*\y) {$\indfeat^{(4)}_{\y}$};
   	\foreach \name / \y in {3}
	    \path[yshift=0.5cm]
    	    node[hidden neuron, minimum size=\nodesize] (H4-\name) at (4*\layersep,-\nodesep*4) {$\indfeat^{(4)}_\infty$};   
...
\node[annot,below of=H4-2, node distance=1.15cm] {$\vdots$};
\end{tikzpicture}
%\end{minipage}
%&
%\usebox{\deepkernels}
...
\centering
\begin{tabular}{cc}
\hspace{-1cm}\includegraphics[width=0.6\columnwidth, clip, trim = 0cm 0cm 0cm 0.61cm]{figures/deep_kernel} &
\hspace{-1cm}\includegraphics[width=0.6\columnwidth, clip, trim = 0cm 0cm 0cm 0.61cm]{figures/deep_kernel_draws} \\
...
\frametitle{A simple fix}
%\begin{itemize}
%	\item Following a suggestion from {\color{blue!80} Neal (1995)}, we connect the inputs $\vx$ to each layer:
%
\begin{align*}
%k_1(\vx, \vx') & = \exp \left( -\frac{1}{2} ||\vx - \vx'||_2^2 \right) \\
& k_{L+1}(\vx, \vx') = \nonumber \\
...
\frame[plain]{
\frametitle{Infinitely deep kernels, take two}
\begin{itemize}
...
\item Can be seen as a ``simpler'' version of Bayesian neural nets
\item Two equivalent architectures.
...
\frame[plain]{
\frametitle{Deep GPs as nonparametric nets}
...
%    \tikzstyle{input neuron}=[neuron, minimum size=\nodesize];
    ...
    % Draw the hidden layer nodes
    \foreach \name / \y in {1,...,\numhidden}
        \path[yshift=0] node[hidden neuron] (H-\name) at (\layersep,-\nodesep*\y) { \neuronfunc{\y}{0}};

    % Draw the hidden layer nodes
    \foreach \name / \y in {1,...,\numhidden}
        \path[yshift=0] node[hidden neuron] (H2-\name) at (2*\layersep,-\nodesep*\y) {\neuronfunc{\y}{4}};
...
%    \node[annot,below of=H2-\numhidden, node distance=\upnodedist, text width = 2cm] {$\vf^{(1:2)}(\vx)$};
    \node[annot,above of=O-1, node distance=\upnodedist] {Targets};
    \node[annot,below of=O-\numdims, node distance=\upnodedist, text width = 1cm] {$\mathbf{t}$};
\end{tikzpicture}
...
\begin{itemize}
	...
	\item In this talk we only consider noiseless functions.
...
\frame[plain]{
\frametitle{Deep GPs as infinitely wide parametric nets}

%A neural net with fixed activation functions corresponding to a 3-layer deep \gp{}
\null\hspace*{-1cm}\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep, scale=0.9]
    \tikzstyle{every pin edge}=[<-,shorten <=1pt]
    \tikzstyle{neuron}=[circle,fill=black!25,minimum size=17pt,inner sep=0pt]
    \tikzstyle{annot} = [text width=4em, text centered]
...
\begin{itemize}
	\item Infinitely-wide fixed feature maps alternating with finite linear information bottlenecks:
	$$
\vh^{(\ell)}(\vx) = \sigma \left( \vb^{(\ell)} + \left[ \mathbf{V}^{(\ell)} \mathbf{W}^{(\ell-1)} \right] \vh^{(\ell-1)}(\vx) \right) $$
\end{itemize}
...
\includegraphics[trim=4mm 1mm 4mm 2.5mm, clip, width=0.475\columnwidth]{../figures/spectrum/layer-#1}}%


\frame[plain]{
\frametitle{Analysis of Jacobian}
...
\begin{tabular}{ccc}
& 2 Layers & 6 Layers \\
\begin{sideways} { \quad \small Normalized singular value} \end{sideways} & \hspace{-0.2in} \spectrumpic{2} & \hspace{-0.1in} \spectrumpic{6} \\
...
\begin{itemize}
	\item Lemma from paper:  The Jacobian of a deep GP is a product of \iid random Gaussian matrices.
	\item Output only changes in w.r.t. one direction as net deepens.	
\end{itemize}
}
...
\frame[plain]{
\frametitle{A different architecture (show video)}
\begin{itemize}
...
\begin{itemize}
	...
	\item GPs are strange because any one feature doesn't matter.
...
\frame[plain]{
\frametitle{Dropout on GP inputs}
\centering
	\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
%    \tikzstyle{interarrows}=[->, thick, black!50]
    \tikzstyle{every pin edge}=[<-,shorten <=1pt]
...
%	\vspace{\baselineskip}
	...
	\item Example 1: Increasing depth makes net outputs change in fewer input directions.
